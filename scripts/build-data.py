...
# v1.7: routes are now data (OTA-updatable). data/routes.json is the canonical
# source; build-data.py regenerates js/routes.js (bundled fallback) from it AND
# ships routes in content.json so route edits go live over OTA (no app build).
ROUTES_JSON_FILE = os.path.join(PROJECT_ROOT, 'data', 'routes.json')
ROUTES_JS_FILE = os.path.join(PROJECT_ROOT, 'js', 'routes.js')
...
def load_routes():
    """Load canonical route defs + paths from data/routes.json.
    Shape: [{id, name, desc, color, ids, distance, segments:[{from,to,path}]}]."""
    if not os.path.exists(ROUTES_JSON_FILE):
        return []
    with open(ROUTES_JSON_FILE, encoding='utf-8') as f:
        return json.load(f)


def generate_routes_js(routes):
    """Regenerate js/routes.js (bundled ROUTE_DEFS + ROUTE_PATHS fallback) from
    data/routes.json. app.js imports both; content.js overlays newer OTA routes
    on top at boot (same mechanism as murals/pois/YEARS)."""
    defs = [{'id': r['id'], 'name': r['name'], 'desc': r.get('desc', ''),
             'color': r.get('color', '#999'), 'ids': r.get('ids', [])} for r in routes]
    paths = {r['id']: {'distance': r.get('distance', 0), 'segments': r.get('segments', [])}
             for r in routes}
    lines = [
        '// GENERATED by scripts/build-data.py from data/routes.json — do not hand-edit.',
        '// Edit routes in tools/route-editor.html; apply its exports to data/routes.json,',
        '// then run build-data.py. Routes ship over OTA via content.json (see content.js).',
        'export const ROUTE_DEFS = ' + json.dumps(defs, ensure_ascii=False) + ';',
        'export const ROUTE_PATHS = ' + json.dumps(paths, ensure_ascii=False, separators=(',', ':')) + ';',
        '',
    ]
    with open(ROUTES_JS_FILE, 'w', encoding='utf-8') as f:
        f.write('\n'.join(lines))


def build_content_manifest(murals, config, pois=None, routes=None):
    ...
    content_core = {'schemaVersion': 1, 'murals': mural_dicts, 'pois': poi_dicts,
                    'YEARS': years, 'routes': routes}
    ...
    manifest = {
        ...
        'hash': content_hash,
        'counts': {'murals': len(mural_dicts), 'pois': len(poi_dicts), 'routes': len(routes)},
        'YEARS': years,
        'murals': mural_dicts,
        'pois': poi_dicts,
        'routes': routes,
...
def main():
    ...
    if dry_run:
        ...
        # v1.7: regenerate the bundled routes fallback from data/routes.json
        routes = load_routes()
        generate_routes_js(routes)
        print(f"✓ Generated {ROUTES_JS_FILE} ({len(routes)} routes)")

        # v1.5: also emit the OTA content manifest + bundled-version marker
        manifest = build_content_manifest(murals, config, pois, routes)
        ...
        print(f"✓ Generated {CONTENT_JSON_FILE} (v{manifest['version']}, hash {manifest['hash']}, "
              ...
              f"+ {manifest['counts']['routes']} routes)")
